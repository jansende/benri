...
//    if (new.empty) %No new elements. We are at the start. Add the first
//    element from
//                   %old to new and start sorting.
//        ret insertion_sort(new.append(old.first), comp, old.rest)
//    else if (!old.empty) %We have old elements left. Add the first element of
//    the old
//                         %list to new and sort it.
//        ret insertion_sort(insert_sort_swap(list(), comp,
//        new.prepend(old.first)),
//                           comp, old.rest)
...
//    else %if (old.size > 2)
//       if (comp(old.first, old.second)) %the elements are sorted. add them to
//       new.
//           ret new.append(old)
//       else                             %unsorted but fixable. swap and
//       continus
...
// Two elements to sort left. If they are in the wrong order, swap them. We are
// done.
template <class... NewTs, template <class, class> class Comp, class FirstOldT,
          class SecondOldT>
struct insert_sort_swap_impl<sorted_list<NewTs...>, Comp, list<FirstOldT, SecondOldT>>
{
    using type = std::conditional_t<Comp<FirstOldT, SecondOldT>::value,
                                    sorted_list<NewTs..., FirstOldT, SecondOldT>,
                                    sorted_list<NewTs..., SecondOldT, FirstOldT>>;
};
// More than two elements left to sort. If they are in the wrong order, swap
// them. Continue.
template <class... NewTs, template <class, class> class Comp, class FirstOldT,
          class SecondOldT, class... OldTs>
struct insert_sort_swap_impl<sorted_list<NewTs...>, Comp,
                             list<FirstOldT, SecondOldT, OldTs...>> :
    std::conditional_t<
        Comp<FirstOldT, SecondOldT>::value,
        insert_sort_swap_impl<sorted_list<NewTs..., FirstOldT, SecondOldT, OldTs...>,
                              Comp, list<>>,
        insert_sort_swap_impl<sorted_list<NewTs..., SecondOldT>, Comp,
...
// No element has been sorted yet. (New list is empty.) Add the first old
// element to the new list, and start sorting.
...
// Old list is not empty. Swap/Merge the first old element into the new list,
// and keep sorting.
...
struct insertion_sort_impl<sorted_list<FirstNewT, NewTs...>, Comp,
                           list<FirstOldT, OldTs...>> :
    insertion_sort_impl<insert_sort_swap<Comp, list<FirstOldT, FirstNewT, NewTs...>>,
                        Comp, list<OldTs...>>
{
};
...
// Basic tests
static_assert(std::is_same<sorted_list<>, insertion_sort<list<>>>::value,
              "type::insertion_sort<list<>> is list<>.");
static_assert(std::is_same<sorted_list<int>, insertion_sort<list<int>>>::value,
              "type::insertion_sort<list<int>> is list<int>.");
static_assert(
    std::is_same<sorted_list<int, double>, insertion_sort<list<int, double>>>::value,
    "type::insertion_sort<list<int, double>> is list<int, double>.");
static_assert(
    std::is_same<sorted_list<int, double>, insertion_sort<list<double, int>>>::value,
    "type::insertion_sort<list<double, int>> is list<int, double>.");
static_assert(std::is_same<sorted_list<int, int, double>,
                           insertion_sort<list<int, double, int>>>::value,
              "type::insertion_sort<list<int, double, int>> is list<int, int, double>.");
static_assert(std::is_same<sorted_list<int, float, double>,
                           insertion_sort<list<double, float, int>>>::value,
              "type::insertion_sort<list<double, float, int>> is list<int, "
              "float, double>.");
#pragma endregion
...
static_assert(std::is_same<sorted_list<>, sort<list<>>>::value,
              "type::sort<list<>> is list<>.");
static_assert(std::is_same<sorted_list<int>, sort<list<int>>>::value,
              "type::sort<list<int>> is list<int>.");
static_assert(std::is_same<sorted_list<int, double>, sort<list<int, double>>>::value,
              "type::sort<list<int, double>> is list<int, double>.");
static_assert(std::is_same<sorted_list<int, double>, sort<list<double, int>>>::value,
              "type::sort<list<double, int>> is list<int, double>.");
static_assert(
    std::is_same<sorted_list<int, int, double>, sort<list<int, double, int>>>::value,
    "type::sort<list<int, double, int>> is list<int, int, double>.");
static_assert(
    std::is_same<sorted_list<int, float, double>, sort<list<double, float, int>>>::value,
    "type::sort<list<double, float, int>> is list<int, float, double>.");
#pragma endregion
} // namespace type
} // namespace benri