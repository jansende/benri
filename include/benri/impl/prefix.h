...
{
    static_assert(
        type::detect_if<T, type::has_value> || type::detect_if<T, type::is_std_ratio>,
        "T has to either be std::ratio or a struct with a ::value attribute.");
...
// the value stored in Power. (Can only handle integer values for Power.)
...
{
    static_assert(type::detect_if<Prefix, type::is_prefix>,
...
{
    static_assert(type::all_true<type::detect_if<Elements, type::is_prefix>...>,
                  "All Elements have to be pre.");
    static_assert(
        type::all_true<
            type::detect_if<typename Elements::power, type::is_integer_ratio>...>,
        "expand_prefix_list cannot handle roots in the atoms at the moment. Use "
        "runtime_expand_prefix_list instead.");
...
static_assert(
    expand_prefix_list<intmax_t, type::sorted_list<pre<std::ratio<2>, std::ratio<2>>,
                                                   pre<std::ratio<7>>>> == 28,
