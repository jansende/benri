...
struct unit
{
    static_assert(type::detect_if<Dimension, type::is_sorted_list>, "Dimension has to be a sorted_list.");
...
template <class LDimension, class LPrefix, class RDimension, class RPrefix>
constexpr bool is_compatible_v = is_compatible<LDimension, LPrefix,
                                               RDimension, RPrefix>::value;

template <class L, class R>
using is_compatible_with = typename std::enable_if_t<
    is_compatible_v<typename L::dimension, typename L::prefix,
                    typename R::dimension, typename R::prefix> ||
    is_compatible_v<typename R::dimension, typename R::prefix,
                    typename L::dimension, typename L::prefix>>;
