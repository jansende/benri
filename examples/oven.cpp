...
int main()
{
    ...
    // set_oven(250_celsius);        //Will not compile because we have no zero
    // point.
    set_oven(zero_point(celsius) + 250_celsius); // Works fine.

    // we can be safe from the following:
    // set_oven(temperature_update()); //Will not compile. (Otherwise the oven
    // would now be at 10°C.)
    set_oven(new_temperature()); // Will compile, and set the temperature to
                                 // 210°C.
}